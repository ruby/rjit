...
class BindingGenerator
  ...
  def generate_node(node, sizeof_type: nil)
    ...
    when :struct, :union
      ...
      #
      # However, in recent LLVM like llvm-18, it may give "union (unnamed at /path/to/vm_core.h:1158:9)",
      # so use an empty string like before in such cases.
      node_spelling = node.spelling
      node_spelling = '' if node_spelling.match?(/\A(struct|union) \(unnamed at .+\)\z/)

      buf = +"CType::#{node.kind.to_s.sub(/\A[a-z]/, &:upcase)}.new(\n"
      buf << "  \"#{node_spelling}\", Primitive.cexpr!(\"SIZEOF(#{sizeof_type || node.type})\"),\n"
